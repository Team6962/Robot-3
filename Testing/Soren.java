...
public class Robot extends TimedRobot {
    ...
    public void robotInit() {
        ...
        // FindTarget.setup();
        // FindBall.readCalibrationData("calib-logitech.mov-720-30-calib.txt",
        ...
        // continue;
        // ballAngleValue[0] = FindBall.getBallValue(source, WINDOW_WIDTH,
        // WINDOW_HEIGHT, cameraMatrix,
        // distCoeffs);
    ...
    public void autonomousPeriodic() {
        ...
        if (now - start > 10000) { // stop after ten seconds
    ...
    public String getColor() {
        ...
        String colorString;
        double range = 0.05;
        Color detectedColor = colorSensor.getColor(); // Asks the sensor for the colour, following section interprets
                                                      // the return.
        if ((detectedColor.red <= 0.1551 + range && detectedColor.red >= 0.1551 - range)
                && (detectedColor.green <= 0.4444 + range && detectedColor.green >= 0.4444 - range)
                && (detectedColor.blue <= 0.4001 + range && detectedColor.blue >= 0.3901 - range)) {

            colorString = "Red";

        } else if ((detectedColor.red <= 0.5173 + range && detectedColor.red >= 0.4073 - range)
                && (detectedColor.green <= 0.3488 + range && detectedColor.green >= 0.3488 - range)
                && (detectedColor.blue <= 0.134 + range && detectedColor.blue >= 0.134 - range)) {

            colorString = "Blue";

        } else if ((detectedColor.red <= 0.1899 + range && detectedColor.red >= 0.1899 - range)
                && (detectedColor.green <= 0.5598 + range && detectedColor.green >= 0.5598 - range)
                && (detectedColor.blue <= 0.2501 + range && detectedColor.blue >= 0.2501 - range)) {

            colorString = "Yellow";

        } else if ((detectedColor.red <= 0.3271 + range && detectedColor.red >= 0.3271 - range)
                && (detectedColor.green <= 0.5385 + range && detectedColor.green >= 0.5385 - range)
                && (detectedColor.blue <= 0.134 + range && detectedColor.blue >= 0.134 - range)) {
    ...
    public static int mode(ArrayList<Integer> il) {
        ...
        for (int k : occurences)
            if (k == largestIn(occurences))
                d++;
    ...
    public void teleopPeriodic() {
        ...
        if (!lastColor.equals(colorString) && (colorString.equals("Red") || colorString.equals("Blue")
                || colorString.equals("Yellow") || colorString.equals("Green"))) {
        ...
        if (joystick0.getRawButton(7))
            pullIn = false;
        if (joystick0.getRawButton(8))
            pullIn = true;
        if (pullIn && drawerIn.get())
            drawer.set(0.7);
        else if (!pullIn && drawerOut.get())
            drawer.set(-0.7);
        else
            drawer.set(0);
